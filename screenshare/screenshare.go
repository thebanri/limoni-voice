...
func buildLinuxBroadcastCommand(opt BroadcastOptions, targetURL string, onCancel ...func()) (string, []string, *os.File, func(), error) {
	...
	if p, err := FindExecutable("ffmpeg"); err == nil {
		...
		args = append(args,
			...
			"-preset", "ultrafast",
			"-tune", "zerolatency",
			"-x264-params", fmt.Sprintf("keyint=%d:min-keyint=%d:qpmin=18:qpmax=38:scenecut=0:no-scenecut=1:sync-lookahead=0:rc-lookahead=0:sliced-threads=1:repeat-headers=1:me=hex:subme=2:merange=16:aq-mode=1", fps, fps),
			"-crf", "22",
			"-b:v", bitrate,
			"-maxrate", maxRate,
			"-bufsize", maxRate,
	...
	case "windows":
		...
		if targetHwnd != 0 {
			...
			args = []string{
				...
				"-preset", "ultrafast",
				"-tune", "zerolatency",
				"-x264-params", fmt.Sprintf("keyint=%d:min-keyint=%d:qpmin=18:qpmax=38:scenecut=0:no-scenecut=1:sync-lookahead=0:rc-lookahead=0:sliced-threads=1:repeat-headers=1:me=hex:subme=2:merange=16:aq-mode=1", winFps, winFps),
				"-crf", "23",
				"-b:v", winBitrate,
				"-maxrate", winMaxRate,
				"-bufsize", winMaxRate,
