...
- **Gürültü Bastırma**: Çok kademeli filtre sistemi (Kapalı / Hafif / Orta / Agresif)
...
- **Windows**: Pencere bazlı seçim ile GDI/DXGI capture
- **macOS**: Yerel sistem portalı ile entegrasyon
- **MPV Player** ile düşük gecikmeli izleme deneyimi
...
> **Not:** Limoni Voice tamamen terminal içinde çalışır — burada gördüğünüz her şey ANSI/Unicode ile çizilmiştir.
...
| `C` | Oda anahtarını panoya kopyala |
| `G` | Yeni oda anahtarı üret |
...
| `D` | 🔇 Sağırlaştır (tüm sesleri kapat) |
| `N` | 🔊 Gürültü filtresi modunu değiştir |
| `V` | 🖥️ Ekran paylaşımını başlat / durdur |
| `W` | 👁️ Ekran yayınını izle |
| `C` | 📋 Oda kodunu kopyala |
| `+` / `-` | 🔉 Mikrofon ses seviyesini ayarla |
| `T` | 🧪 Ses test modalı |
| `Esc` | Odadan ayrıl |

### P2P Mesh Testi (Yerel Ağ)

```bash
# Terminal 1 — Oda oluştur
./limoni-voice
# → [Enter] ile odayı başlat
# → Oda anahtarını not al (örn: 9421-azure-wave)

# Terminal 2 — Odaya katıl
./limoni-voice
# → [Tab] ile "Odaya Katıl" alanına geç
# → Oda anahtarını yaz ve [Enter]'a bas

# ✅ Her iki terminal anında bağlanır, ses akışı başlar!
```
...
Ekran paylaşımı özelliği için harici araçlar gerekebilir:
...
Test kapsamı:
- ✅ Oda kodu üretimi ve normalizasyon
- ✅ Ses motoru başlatma ve kontrol
- ✅ AES-256-GCM şifreleme / çözme
- ✅ Paket serialization / deserialization
- ✅ VU-Meter ve UI widget render
- ✅ Relay sunucu oda yönetimi

---

## 🐳 Self-Hosted Relay Sunucusu

Kendi relay sunucunuzu barındırmak isterseniz:
...
PORT=8080 ./relay-server
```

Ardından istemcide özel relay URL'nizi kullanabilirsiniz.

Varsayılan relay: `wss://limoni-voice-production.up.railway.app/ws`
...
```bash
git clone https://github.com/<kullanici>/limoni-voice.git
cd limoni-voice

# Limoni TUI framework'ünü aynı dizine klonlayın
git clone https://github.com/thebanri/limoni.git ../Limoni
...
Üçüncü parti bileşenler kendi lisansları altında dağıtılmaktadır:
